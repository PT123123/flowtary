...
fn collect_match_indices(
    ...
    // The same limit-before-filter hazard applies to `filter_mode`
    // (FilesOnly/DirsOnly): when the first `limit` name-matches are all the
    // excluded kind, valid matches past the cutoff would be dropped by the
    // post-merge `apply_filter`. Filter here, before the cutoff.
    let keep = |rec: &CompactRecord, name: &str, buf: &mut Vec<u8>| -> bool {
        matches(name, buf)
            && passes_filter_mode(rec.is_directory(), filter_mode)
            && filters.matches_record(rec, &drive.names, buf, drive.fold)
    };
    match candidates {
        None => {
            let mut out = Vec::new();
